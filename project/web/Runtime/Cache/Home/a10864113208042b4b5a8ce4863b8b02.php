...
<div class="container" >
 <div class="row">
  <div class="col-md-4 pull-left">
   <div class="thumbnail s1" style="width:364px;">
	  <div id="small"style="width:360px;height:360px;position: absolute;left:20px;top:10px;">
		  <img id="simg" style="width:350px;height:360px;" src=" <?php echo ($info["gpic"]); ?>">
		</div>
		<div class="caption"style="margin-top:410px;">
		  <ul id="uimg" style="position:absolute;left:40px;top:380px;"class="row mt10">
		   <?php foreach ($res as $k => $v): ?>
				<li style="margin-left:5px;" class="pull-left s3">
				  <img style="width:50px;height:45px;margin-left:10px;" src="<?php echo ($v["iname"]); ?>">
				</li>
		   <?php endforeach ?>
		  </ul>
		</div>
	...
	<div class="col-md-7 pull-right">
	  <div id="big" style="width:360px;height:360px;
		position:absolute;left:206px;
		top:10px;overflow:hidden;
		display:none;z-index:999">
		 <img id="bimg" style="position:absolute;" src="<?php echo ($info["gpic"]); ?>" >
	  </div>
	<div class="row">
	   <h4 class="col-md-4"style="line-height:40px;width:200px;background:#e1e1e1;">商品名称:</h4>
	   <h4 style="margin-left:140px;line-height:40px;background:#e1e1e1;color:#f46;"><?php echo ($info["gname"]); ?></h4><br>
	</div>
	<div class="row">
		<h4 class="col-md-4">商品简介:</h4>
		<h4 style=""><?php echo ($info["msg"]); ?></h4><br>
	</div>
	<div class="row">
		<h1 class="red col-md-6 pull-left">
		  <p style="color:#f46;"><span class="glyphicon glyphicon-yen"></span> 
		  <?php echo ($info["price"]); ?></p>
		</h1>
		<h1 class="col-md-4 pull-right">
		  <a href="<?php echo U('Home/Contents/zan',array('id'=>$info['id']));?>?zan=<?php echo ($info["zan"]); ?>" class="red zan"><span class="glyphicon glyphicon-heart"style="color:#f46;"></span><?php echo ($info["zan"]); ?></a><br><br>
		</h1>
	  </div>

	  <form action="<?php echo U('Home/Cart/index');?>" method="get">
		<!-- 隐藏域商品的ID -->
		...
		<div class="row mt20">
		  ...
		  <div class="col-md-4">
			...
			<span class="col-md-3 h4">库存:<?php echo ($info["stock"]); ?></span>
			<span class="col-md-3 h4">销量:<?php echo ($info["sale"]); ?></span><br><br>
		</div><!-- row mt20 -->
		<div class="row mt30">
		  ...
		  <div class="col-md-12 mt10">
			...
			<a id="adda" href="<?php echo U('Home/Cart/add',array('goods_id'=>$info['id']));?>?qty=1" class="btn btn-lg btn-info pull-right"  onclick="if(confirm('你确定要加入购物车吗?')==false) return false;"><span class="glyphicon glyphicon-shopping-cart"> 加入购物车</span></a>
	  ...
	  <div class="caption">
		  ...
		  <?php foreach ($res as $k => $v): ?>
			<li class="pull-left s3">
			  <img style="width:800px;center;margin-left:200px;margin-top:20px;" src="<?php echo ($v["iname"]); ?>" ><br><br>
			</li>
		   <?php endforeach ?>
		  </ul>
		</div>
 ...
 <script type="text/javascript" src="/Public/Home/js/jquery-1.8.3.min.js"></script>
  ...
  // 获取元素对象
		...
		simg.onmousemove = function(e){
			var e = e || event;
      //获取鼠标在图片上的相对坐标
      var x = e.clientX - simg.offsetLeft;
      var y = e.clientY - simg.offsetTop;
     // alert(x);
      //大图与小图的比例
        big.scrollTop = y * 4 - 380;
        big.scrollLeft = x * 4 - 680;
		...
		for(var i=0;i<imgs.length;i++){
			imgs[i].onclick = function(){
				//获取src属性 控
				// var src = this.src;//获取的是绝对路径
				var src = this.getAttribute('src');//获取相对路径
				// alert(src);
				//设置src属性
				simg.src = src;
				bimg.src = src;
...
//
  var href = '<?php echo U('Home/Cart/add',array('goods_id'=>$info['id']));?>?qty='
  //function changesrc (obj) {
	// alert(obj.src);
	//获取大图的src
	//var small = document.getElementById('small');
	//修改大图的src
	//small.src = obj.src.replace('50_','');
 // }
  function plus(){
	var ipt = document.getElementById('num');
	ipt.value = parseInt(ipt.value) + 1;
	if (ipt.value >= <?php echo ($info["stock"]); ?> ) {
	  ipt.value = <?php echo ($info["stock"]); ?>;
	}
	<?php  $stock="select stock from hs_goods where `id`='$goods_id' "; $kc=$rows['0']['stock']; ?>
