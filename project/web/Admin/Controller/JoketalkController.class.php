...
class JoketalkController extends Controller {
    //显示开心一刻列表
    public function index(){
       ...
       // var_dump($count);
        ...
        //执行查询
        //var_dump($where);die();
         $jokes=$joke->join('right join hs_joke_talk on hs_joke.id=hs_joke_talk.joke_id')->where($where)->limit($limit)->order('talk_time desc')->select();
       // $users=$joke->where($where)->order('joke_time desc')->limit($limit)->select();
       //echo $joke->_sql();die();s
       // 分页显示输出
        $pages = $Page->show();
       //分配变量
